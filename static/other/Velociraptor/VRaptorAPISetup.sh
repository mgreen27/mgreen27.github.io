...
# Assumes wget installed tested in CentOS only
# Additional forensic tools required installation
# Variables:
...
if [ ${os^^} == "CENTOS" ]; then
	...
	echo -e "\nPlaceholder - Ubuntu not supported for Velociraptor API POC\n"
	exit
else
...
# User message
echo -e "Velociraptor-Processing Service control:"
